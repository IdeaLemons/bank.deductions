...
function CurrentProjectID() {
	...
	return "{163802B9-268A-4AFB-8FD6-7A7D18262A99}";
}
...
function ew_GetFileUploadUrl($fld, $val, $resize = FALSE, $encrypt = EW_ENCRYPT_FILE_PATH, $urlencode = TRUE) {
	if (!ew_EmptyStr($val)) {
		...
		if ($encrypt) {
			...
			$fn = "ewfile12.php?t=" . ew_Encrypt($fld->TblName, $key) ."&fn=" . ew_Encrypt($path . $val, $key);
			if ($resize)
				$fn .= "&width=" . $fld->ImageWidth . "&height=" . $fld->ImageHeight;
		} elseif ($resize) {
			$fn = "ewfile12.php?t=" . rawurlencode($fld->TblName) . "&fn=" . ew_UrlEncodeFilePath($path . $val) .
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($ValidateUser) {
						...
						$this->setCurrentUserName($rs->fields('PF')); // Load user name
...
include_once($EW_RELATIVE_PATH . "phpmailer529/PHPMailerAutoload.php");
...
function ew_Empty($value) {
	...
	//***return is_null($value); // *** To be verified
}
