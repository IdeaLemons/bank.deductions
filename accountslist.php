...
class caccounts_list extends caccounts {
	...
	function __construct() {
		...
		$this->FilterOptions->TagClassName = "ewFilterOption faccountslistsrch";
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->Acc_ID->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->Acc_ID->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function RestoreFilterList() {
		...
		$this->Acc_NO->AdvancedSearch->SearchCondition = @$filter["v_Acc_NO"];
		$this->Acc_NO->AdvancedSearch->SearchValue2 = @$filter["y_Acc_NO"];
		$this->Acc_NO->AdvancedSearch->SearchOperator2 = @$filter["w_Acc_NO"];
		$this->Acc_NO->AdvancedSearch->Save();
	...
	function BasicSearchSQL($arKeywords, $type) {
		$sWhere = "";
		$this->BuildBasicSearchSQL($sWhere, $this->Bank_ID, $arKeywords, $type);
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->Acc_ID->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event);'>";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"faccountslistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"faccountslistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.faccountslist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
...
var CurrentForm = faccountslist = new ew_Form("faccountslist", "list");
...
<?php } ?>
<?php
	$bSelectLimit = $accounts_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($accounts_list->TotalRecs <= 0)
			$accounts_list->TotalRecs = $accounts->SelectRecordCount();
	} else {
		if (!$accounts_list->Recordset && ($accounts_list->Recordset = $accounts_list->LoadRecordset()))
...
// Header row
$accounts_list->RowType = EW_ROWTYPE_HEADER;
...
<?php if ($accounts->PF->Visible) { // PF ?>
	<?php if ($accounts->SortUrl($accounts->PF) == "") { ?>
		<th data-name="PF"><div id="elh_accounts_PF" class="accounts_PF"><div class="ewTableHeaderCaption"><?php echo $accounts->PF->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="PF"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $accounts->SortUrl($accounts->PF) ?>',1);"><div id="elh_accounts_PF" class="accounts_PF">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $accounts->PF->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($accounts->PF->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($accounts->PF->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
...
<?php if ($accounts->Acc_NO->Visible) { // Acc_NO ?>
	<?php if ($accounts->SortUrl($accounts->Acc_NO) == "") { ?>
		<th data-name="Acc_NO"><div id="elh_accounts_Acc_NO" class="accounts_Acc_NO"><div class="ewTableHeaderCaption"><?php echo $accounts->Acc_NO->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Acc_NO"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $accounts->SortUrl($accounts->Acc_NO) ?>',1);"><div id="elh_accounts_Acc_NO" class="accounts_Acc_NO">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $accounts->Acc_NO->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($accounts->Acc_NO->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($accounts->Acc_NO->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
?>
	<?php if ($accounts->PF->Visible) { // PF ?>
		<td data-name="PF"<?php echo $accounts->PF->CellAttributes() ?>>
<span id="el<?php echo $accounts_list->RowCnt ?>_accounts_PF" class="accounts_PF">
...
<?php echo $accounts->Bank_Name->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($accounts->Acc_NO->Visible) { // Acc_NO ?>
		<td data-name="Acc_NO"<?php echo $accounts->Acc_NO->CellAttributes() ?>>
<span id="el<?php echo $accounts_list->RowCnt ?>_accounts_Acc_NO" class="accounts_Acc_NO">
...
<?php if (!isset($accounts_list->Pager)) $accounts_list->Pager = new cPrevNextPager($accounts_list->StartRec, $accounts_list->DisplayRecs, $accounts_list->TotalRecs) ?>
...
<!--first page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $accounts_list->PageUrl() ?>start=<?php echo $accounts_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($accounts_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $accounts_list->PageUrl() ?>start=<?php echo $accounts_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $accounts_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($accounts_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $accounts_list->PageUrl() ?>start=<?php echo $accounts_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($accounts_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $accounts_list->PageUrl() ?>start=<?php echo $accounts_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $accounts_list->Pager->PageCount ?></span>
</div>
