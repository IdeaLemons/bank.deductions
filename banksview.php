...
$banks_view = NULL; // Initialize page object first

class cbanks_view extends cbanks {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["Bank_ID"] <> "") {
			$this->RecKey["Bank_ID"] = $_GET["Bank_ID"];
			$KeyUrl .= "&amp;Bank_ID=" . urlencode($this->RecKey["Bank_ID"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["Bank_ID"] <> "") {
				...
				$this->RecKey["Bank_ID"] = $this->Bank_ID->QueryStringValue;
			} elseif (@$_POST["Bank_ID"] <> "") {
				$this->Bank_ID->setFormValue($_POST["Bank_ID"]);
				$this->RecKey["Bank_ID"] = $this->Bank_ID->FormValue;
			} else {
				$sReturnUrl = "bankslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "bankslist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "bankslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($banks_view)) $banks_view = new cbanks_view();

// Page init
$banks_view->Page_Init();

// Page main
$banks_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$banks_view->Page_Render();
?>
...
var CurrentForm = fbanksview = new ew_Form("fbanksview", "view");

// Form_CustomValidate event
fbanksview.Form_CustomValidate = 
...
fbanksview.ValidateRequired = true;
<?php } else { ?>
fbanksview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fbanksview.Lists["x_Bank_Code"] = {"LinkField":"x_Bank_Code","Ajax":true,"AutoFill":false,"DisplayFields":["x_Bank_Code","","",""],"ParentFields":[],"ChildFields":["x_Name"],"FilterFields":[],"Options":[],"Template":""};
fbanksview.Lists["x_Name"] = {"LinkField":"x_Bank_Code","Ajax":true,"AutoFill":false,"DisplayFields":["x_Name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $banks_view->ExportOptions->Render("body") ?>
<?php
	foreach ($banks_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $banks_view->ShowPageHeader(); ?>
<?php
$banks_view->ShowMessage();
?>
<form name="fbanksview" id="fbanksview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($banks_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $banks_view->Token ?>">
<?php } ?>
...
<?php echo $banks->Bank_Code->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $banks->Branch_Code->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $banks->Name->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $banks->City->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
