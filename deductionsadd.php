...
$deductions_add = NULL; // Initialize page object first

class cdeductions_add extends cdeductions {
	...
	// Page object name
	var $PageObjName = 'deductions_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["Deduction_ID"] != "") {
				...
				$this->setKey("Deduction_ID", $this->Deduction_ID->CurrentValue); // Set up key
			} else {
				$this->setKey("Deduction_ID", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "deductionsview.php")
	...
	function LoadDefaultValues() {
		...
		$this->STARTED->CurrentValue = date("Y/m/d");;
		$this->ENDED->CurrentValue = NULL;
		$this->ENDED->OldValue = $this->ENDED->CurrentValue;
	...
	function RenderRow() {
		...
		$this->NOTES->ViewCustomAttributes = "";
			...
			$sSqlWrk = "SELECT `PF`, `Acc_NO` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld`, `PF` AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `accounts`";
			$sWhereWrk = "";
			ew_AddFilter($sWhereWrk, $sFilterWrk);
			$this->Lookup_Selecting($this->Acc_ID, $sWhereWrk); // Call Lookup selecting
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
	...
	function AddRow($rsold = NULL) {
		...
		// L_Ref
		$this->L_Ref->SetDbValueDef($rsnew, $this->L_Ref->CurrentValue, NULL, FALSE);
		...
		// STARTED
		$this->STARTED->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->STARTED->CurrentValue, 5), NULL, FALSE);

		// ENDED
		$this->ENDED->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->ENDED->CurrentValue, 5), NULL, FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->Deduction_ID->setDbValue($conn->Insert_ID());
				$rsnew['Deduction_ID'] = $this->Deduction_ID->DbValue;
			}
		} else {
...
// Create page object
if (!isset($deductions_add)) $deductions_add = new cdeductions_add();

// Page init
$deductions_add->Page_Init();

// Page main
$deductions_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$deductions_add->Page_Render();
?>
...
var CurrentForm = fdeductionsadd = new ew_Form("fdeductionsadd", "add");

// Validate form
fdeductionsadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($deductions->PF->FldErrMsg()) ?>");
...
fdeductionsadd.Form_CustomValidate = 
...
fdeductionsadd.ValidateRequired = true;
<?php } else { ?>
fdeductionsadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fdeductionsadd.Lists["x_YEAR"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
fdeductionsadd.Lists["x_Acc_ID"] = {"LinkField":"x_PF","Ajax":true,"AutoFill":false,"DisplayFields":["x_Acc_NO","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $deductions_add->ShowPageHeader(); ?>
<?php
$deductions_add->ShowMessage();
?>
<form name="fdeductionsadd" id="fdeductionsadd" class="<?php echo $deductions_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($deductions_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $deductions_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="deductions">
<input type="hidden" name="a_add" id="a_add" value="A">
<div class="ewDesktop">
<div>
<table id="tbl_deductionsadd" class="table table-bordered table-striped ewDesktopTable">
...
<input type="text" data-table="deductions" data-field="x_PF" name="x_PF" id="x_PF" size="6" maxlength="6" placeholder="<?php echo ew_HtmlEncode($deductions->PF->getPlaceHolder()) ?>" value="<?php echo $deductions->PF->EditValue ?>"<?php echo $deductions->PF->EditAttributes() ?>>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("fdeductionsadd", "x_STARTED", "%Y/%m/%d");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("fdeductionsadd", "x_ENDED", "%Y/%m/%d");
</script>
<?php } ?>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $deductions_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</div>
</form>
<script type="text/javascript">
