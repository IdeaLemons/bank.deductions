...
$emp_add = NULL; // Initialize page object first

class cemp_add extends cemp {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["PF"] != "") {
				...
				$this->setKey("PF", $this->PF->CurrentValue); // Set up key
			} else {
				$this->setKey("PF", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "empview.php")
	...
	function AddRow($rsold = NULL) {
		...
		if ($bInsertRow && $this->ValidateKey && strval($rsnew['PF']) == "") {
...
// Create page object
if (!isset($emp_add)) $emp_add = new cemp_add();
...
var CurrentForm = fempadd = new ew_Form("fempadd", "add");

// Validate form
fempadd.Validate = function() {
...
fempadd.ValidateRequired = true;
<?php } else { ?>
fempadd.ValidateRequired = false; 
<?php } ?>
...
<?php $emp_add->ShowPageHeader(); ?>
<?php
$emp_add->ShowMessage();
?>
<form name="fempadd" id="fempadd" class="<?php echo $emp_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($emp_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $emp_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $emp_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</div>
</form>
<script type="text/javascript">
