...
class cbanks extends cTable {
	...
	function __construct() {
		...
		// Bank_ID
		$this->Bank_ID = new cField('banks', 'banks', 'x_Bank_ID', 'Bank_ID', '`Bank_ID`', '`Bank_ID`', 3, -1, FALSE, '`Bank_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->Bank_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Bank_ID'] = &$this->Bank_ID;

		// Bank_Code
		$this->Bank_Code = new cField('banks', 'banks', 'x_Bank_Code', 'Bank_Code', '`Bank_Code`', '`Bank_Code`', 3, -1, FALSE, '`EV__Bank_Code`', TRUE, FALSE, TRUE, 'FORMATTED TEXT', 'TEXT');
		$this->Bank_Code->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Bank_Code'] = &$this->Bank_Code;

		// Branch_Code
		$this->Branch_Code = new cField('banks', 'banks', 'x_Branch_Code', 'Branch_Code', '`Branch_Code`', '`Branch_Code`', 3, -1, FALSE, '`Branch_Code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Branch_Code->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`banks`";
	}
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`Name` ASC";
	}
	...
	function UseVirtualFields() {
		...
		if ($this->Bank_Code->AdvancedSearch->SearchValue <> "" ||
			$this->Bank_Code->AdvancedSearch->SearchValue2 <> "" ||
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('Bank_ID', $rs))
				ew_AddFilter($where, ew_QuotedName('Bank_ID', $this->DBID) . '=' . ew_QuotedValue($rs['Bank_ID'], $this->Bank_ID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->Bank_ID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@Bank_ID@", ew_AdjustSql($this->Bank_ID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("banksview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("banksview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "banksadd.php?" . $this->UrlParm($parm);
		else
			$url = "banksadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("banksedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("banksadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("banksdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "Bank_ID:" . ew_VarToJson($this->Bank_ID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->Bank_ID->CurrentValue)) {
			$sUrl .= "Bank_ID=" . urlencode($this->Bank_ID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = $isPost ? ew_StripSlashes(@$_POST["Bank_ID"]) : ew_StripSlashes(@$_GET["Bank_ID"]); // Bank_ID
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->Branch_Code->EditValue = $this->Branch_Code->CurrentValue;
