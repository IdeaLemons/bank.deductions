...
class caccounts extends cTable {
	...
	function __construct() {
		...
		// Acc_ID
		$this->Acc_ID = new cField('accounts', 'accounts', 'x_Acc_ID', 'Acc_ID', '`Acc_ID`', '`Acc_ID`', 3, -1, FALSE, '`Acc_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->Acc_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Acc_ID'] = &$this->Acc_ID;

		// PF
		$this->PF = new cField('accounts', 'accounts', 'x_PF', 'PF', '`PF`', '`PF`', 3, -1, FALSE, '`EV__PF`', TRUE, FALSE, TRUE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT CONCAT(`PF`,'" . ew_ValueSeparator(1, $this->PF) . "',`Name`,'" . ew_ValueSeparator(2, $this->PF) . "',`NIC`) FROM `emp` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`PF` = `accounts`.`PF` LIMIT 1) AS `EV__PF`, (SELECT CONCAT(`Name`,'" . ew_ValueSeparator(1, $this->Bank_ID) . "',`City`) FROM `banks` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`Bank_ID` = `accounts`.`Bank_ID` LIMIT 1) AS `EV__Bank_ID` FROM `accounts`" .
			") `EW_TMP_TABLE`";
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`Bank_ID` ASC";
	}
	...
	function UseVirtualFields() {
		...
		if ($this->Bank_ID->AdvancedSearch->SearchValue <> "" ||
			...
			return TRUE;
		if (strpos($sOrderBy, " " . $this->Bank_ID->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		return FALSE;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('Acc_ID', $rs))
				ew_AddFilter($where, ew_QuotedName('Acc_ID', $this->DBID) . '=' . ew_QuotedValue($rs['Acc_ID'], $this->Acc_ID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->Acc_ID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@Acc_ID@", ew_AdjustSql($this->Acc_ID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("accountsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("accountsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "accountsadd.php?" . $this->UrlParm($parm);
		else
			$url = "accountsadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("accountsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("accountsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "Acc_ID:" . ew_VarToJson($this->Acc_ID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->Acc_ID->CurrentValue)) {
			$sUrl .= "Acc_ID=" . urlencode($this->Acc_ID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = $isPost ? ew_StripSlashes(@$_POST["Acc_ID"]) : ew_StripSlashes(@$_GET["Acc_ID"]); // Acc_ID
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		// Acc_ID
		$this->Acc_ID->EditAttrs["class"] = "form-control";
